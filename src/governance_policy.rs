...
impl GovernancePolicyEngine {
    ...
    pub fn evaluate_proposal_compliance(
        ...
        match &proposal.proposal_type {
            GovernanceProposalType::ConstitutionalAmendment(_) => {
                ...
                if proposal.approval_percentage < 75 {
                    ...
                    report.issues.push("Constitutional amendment requires 75% approval".to_string());
            ...
            GovernanceProposalType::EmergencyBrake(_) => {
                ...
                if validators.len() < 3 {
                    report.compliant = false;
                    report.issues.push("Insufficient validators for emergency procedures".to_string());
                    report.risk_level = RiskLevel::High;
                }
            },
            _ => {
                ...
                if proposal.approval_percentage < 50 {
                    report.compliant = false;
                    report.issues.push("Proposal has low approval percentage".to_string());
        ...
        if proposal.proposer_stake < proposal.minimum_stake_threshold {
            report.compliant = false;
            report.issues.push("Insufficient proposer stake".to_string());
        }
    ...
    pub fn apply_policy_change(
        ...
        // 找到策略
        let policy_index = self.policies.iter()
...
mod tests {
    ...
    fn test_policy_engine() {
        ...
        // 评估提案合规性
        let compliance_report = policy_engine.evaluate_proposal_compliance(&test_proposal, &validators);
    ...
    fn test_policy_change() {
        ...
        assert_eq!(updated_policy.parameters.get("quorum_percentage").unwrap(), "25");
