...
fn test_advanced_governance_features() {
    ...
    for prop in &mut governance_engine.proposals {
        if prop.id == proposal_id {
            prop.voting_deadline = std::time::SystemTime::now()
                .duration_since(std::time::UNIX_EPOCH)
                .unwrap()
                .as_secs() - 100; // 设置为100秒前
            prop.votes_for = 1500;
            prop.votes_against = 300;
            prop.total_stake_voted = 1800;
            prop.quorum_met = true;
            prop.approval_percentage = (prop.votes_for * 100) / (prop.votes_for + prop.votes_against);
            break;
        }
    }
    ...
    println!("✓ Compliance report - Compliant: {}, Risk Level: {:?}", 
    ...
    assert_eq!(updated_policy.parameters.get("quorum_percentage").unwrap(), "30");
...
fn test_constitutional_rule_management() {
    ...
    println!("✓ Added constitutional rule, total rules: {}", updated_rules_count);
